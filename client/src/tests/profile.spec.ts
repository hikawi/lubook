...
describe("/profile route", () => {
  ...
  it("displays an error view if profile not found", async () => {
    ...
    await expect
      .element(page.getByText("that profile doesn't exist"))
      .toBeVisible();
  });
  ...
  it("shows bio if there is bio in the data", async () => {
    getJsonMock.mockResolvedValueOnce(
      new Response(JSON.stringify({ bio: "Weehee" }), { status: 200 }),
    );
    render(ProfileView);
  ...
  it("displays correct view if viewing others", async () => {
    getJsonMock.mockResolvedValueOnce(
      new Response(JSON.stringify({ self: false }), { status: 200 }),
    );
    render(ProfileView);

    await expect.element(page.getByText("Block")).toBeVisible();
    await expect
      .element(page.getByText("Follow", { exact: true }))
      .toBeVisible();
  });
});
